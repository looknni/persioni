...
##### ntpdate 0.gentoo.pool.ntp.org; timedatectl set-timezone Asia/Chongqing # /etc/systemd/timesyncd.conf ? /etc/chrony.conf ? /etc/ntpsec/ntp.conf # server edu.ntp.org.cn iburst
##### sudo flatpak remote-modify flathub --url=https://mirror.sjtu.edu.cn/flathub
##### [On Flatpak error fonts](https://flatpak.org/setup/) ? cp -r /etc/fonts/* org.example.com/config/fontconfig
##### `https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb` # beta|unstable x86_64.rpm # webrtc #
---
...
ffplay -rtsp_transport tcp -acodec pcm_s16le -fflags nobuffer -flags low_delay -framedrop -analyzeduration 1000000 -probesize 32 rtsp://admin:password@ip:554/Streaming/Channels/101
ffplay -rtsp_transport tcp -acodec pcm_s16le -fflags nobuffer -flags low_delay -framedrop -analyzeduration 1000000 -probesize 32 rtsp://admin:password@ip:554/h264/ch1/main/av_stream
...
GRUB_CMDLINE_LINUX="video=1366x768" # /etc/modprobe.d/blacklist-nouveau.conf blacklist nouveau
...
make distclean
make menuconfig # dnsmasq-full libustream-wolfssl wpad-wolfssl vim aircrack-ng openssl-util hcxdumptool tcpdump openssh-server openssh-sftp-server openssh-client unbound-daemon
make download -j4;make -j4 V=s
```
##### find / \( -path /proc -o -path /run \) -prune -o -type l ! -exec test -e {} \; -print
``` 
PS1='\[\e[0;37m\]\l \[\e[0;32m\]\A \[\e[1;95m\]\W \[\e[1;33m\]\$ \[\e[0m\]'
export LC_TIME="de_DE.UTF-8" # zh_CN zh_HK.UTF-8 en_GB.UTF-8 en_AU.UTF-8
