...
>> - /biosboot ~($\lim_{31k\rightarrow\infty}1M$).../boot/efi ~(1M+$\vec{6M}\cdot\vec{n}$System+40M).../boot ~(300M+$\vec{6M}\cdot\vec{n}$Kernel).../ ~($\frac{\infty}{0.29}$).../swap ~($\sqrt[i=2]{RAM}$) . /home  ~($\frac{\infty}{0.69}$).. /usr . /var . /opt . /tmp
